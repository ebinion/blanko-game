...
**Alternatives considered**:
- **`Intl.Collator` with `sensitivity: "accent"`** — its "accent" sensitivity is actually accent-sensitive *and* case-sensitive; flipping to `"variant"` is case-sensitive too. There is no built-in mode that matches our rule exactly. Custom normalization is simpler than fighting the collator's options.
